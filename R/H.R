...
sample_info <- read.delim("Counts_H.txt")
...
HS_Entrez <- as.data.frame(elementMetadata(GeneData[GeneData$type == "gene", c(5,6,7)]))
colnames(HS_Entrez) <- c("locus_tag", "EntrezID", "GeneName")

diff_results2 <- merge(diff_results, HS_Entrez, by.x=0, by.y=1, all.x=TRUE)
