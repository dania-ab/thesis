...
gff_file <- "albicans.gff"
GeneData <- rtracklayer::import.gff(gff_file)
CGDID_Entrez <- as.data.frame(elementMetadata(GeneData[GeneData$type == "gene", c(20,6,7)]))
CGDID_Entrez$locus_tag <- gsub(pattern = "CAALFM", replacement = "gene-CAALFM", x = CGDID_Entrez$locus_tag)
CGDID_Entrez$Dbxref <- gsub(pattern = "GeneID:", replacement = "", x = CGDID_Entrez$Dbxref)
colnames(CGDID_Entrez) <- c("locus_tag", "EntrezID", "GeneName")

diff_results2 <- merge(diff_results, CGDID_Entrez, by.x=0, by.y=1, all.x=TRUE)
